...
- A runner, projection, Skill-drift or Host-transport failure is not a product failure. Preserve it, apply the exact fast-path failure class, and rerun only the allowed affected scope. An unrefined product-behavior failure stops. A deterministically proven `PRODUCT_DEFECT_REPAIRABLE` failure may be handed to an independently authorized campaign-level bounded recovery controller, but this Skill never edits source or creates the append-only successor Candidate. Every successor still requires impact closure and the complete Target-required matrix.
