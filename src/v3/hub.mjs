...
export function buildHubSnapshot(home) {
  ...
  const records = assets.map((record) => ({ kind: record.asset.kind, id: record.asset.metadata.id, name: record.asset.metadata.name, version: record.asset.metadata.version, lifecycle: record.asset.metadata.lifecycle, description: record.asset.metadata.description, domain: record.asset.spec.classification?.domain, semanticRequirements: record.asset.kind === "HarnessBundle" && record.asset.spec.semanticRequirements ? { schema: record.asset.spec.semanticRequirements.schema, foundationDigest: record.asset.spec.semanticRequirements.foundationDigest, requirementsDigest: record.asset.spec.semanticRequirements.requirementsDigest, requiredConceptCount: record.asset.spec.semanticRequirements.requiredConcepts.length, prohibitedConceptCount: record.asset.spec.semanticRequirements.prohibitedConcepts.length } : null, digest: record.digest, file: record.file }));
  const snapshot = {
