...
The canonical asset API namespace remains `harness.evopilot.io/v3`; structured feedback uses `feedback.evopilot.io/v1`, controlled comparison uses `comparison.evopilot.io/v1`, and Agent operation uses `evopilot-harness-agent-operations/v1`. Optional control-plane projections are exports, not source assets. Engine `4.1.0` retains the v3 JSON CLI, Asset v3, Workspace, Catalog, Registry, EvaluationPack v1/v2, and v4 Session read compatibility; approval still requires valid Delta/Evaluation closure, a current Proposal Review Report, and a current comparison snapshot when one is bound. See [ADR 0002](adr/0002-agent-native-harness-operations.md), [ADR 0003](adr/0003-controlled-comparative-evidence.md), and [v2 Architecture Compatibility](v2-compatibility.md).
