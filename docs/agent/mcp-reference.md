...
| `run_engine_diagnostic` with `semantic.foundation.inspect`, `semantic.candidates.inspect`, `semantic.grounding.inspect`, or `semantic.compatibility.inspect` | Inspect v4.6 Foundation, candidate extraction, grounding, and Bundle compatibility contracts | Read-only; no Eligibility, Proposal, approval, or publication authority |
| `run_engine_diagnostic` with `pack.scaffold.inspect`, `pack.inspect`, `pack.resolve.inspect`, `pack.benchmark.inspect`, `pack.gold-case.inspect`, `pack.certification.inspect`, `semantic.evidence-adapter.inspect`, `semantic.evidence.import.inspect`, `project-ontology.proposal.inspect`, `project-ontology.snapshot.resolve`, `project-ontology.projections.inspect`, or `project-ontology.skill.inspect` | Inspect v4.7 declarative Pack, reviewed benchmark/Gold Case, inactive evidence, Project Ontology, projection, and Skill contracts | Read-only; no apply, Review, Approval, Publication, installation, activation, or release authority |
| `run_engine_diagnostic` with `semantic.reasoning-profile.inspect`, `semantic.index.inspect`, `semantic.affected-subgraph.inspect`, `semantic.compute.inspect`, `semantic.compute.compare`, `semantic.federation.discover`, `semantic.interoperability.inspect`, `semantic.round-trip.inspect`, `semantic.closure.inspect`, or `semantic.closure.slice` | Inspect v4.8 bounded reasoning, indexes, incremental/full equivalence, read-only federation, projections, terminal closure, and offline consumer slices | Read-only; no external reasoner execution, mutation, approval, publication, installation, activation, or Release authority |
...
Every tool schema recursively rejects unknown fields where the contract is closed. Engine operation inputs pass a second field whitelist, secret-material check, and write-boundary check. Proposal approval, Pack or Project Ontology lifecycle transitions, publication, and planned comparison/calibration/learning mutations are not exposed through the generic diagnostic tool. `run_engine_diagnostic` may inspect or validate evidence inputs and read reports. `comparison.process`, `comparison.ingest|score|rescore`, `calibration.ingest|run`, `learning.ingest|snapshot|run-manifest|score|rescore`, `pack.lifecycle.create|transition`, `project-ontology.proposal.transition`, and `project-ontology.artifact.transition` require a confirmed Session. `catalog.publish`, `ontology.publish`, `policy.publish`, `project-ontology.artifact.publish`, and `semantic.closure.publish` stop after Plan confirmation and require their own operation authorization before the Engine may publish.
